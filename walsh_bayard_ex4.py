...
def cosine_similarity(A,B):
    ...
    # for i=1 ... n compute sums for A_i^2, B_i^2 and A_i*B_i
    n=range(len(A))
    
    for i in n:
       numeratorsum+= A[i] * B[i]
       a_denominatorsum+= (A[i]) ** 2
       b_denominatorsum+= (B[i]) ** 2

    cosine_value = numeratorsum / (math.sqrt(a_denominatorsum) * math.sqrt(b_denominatorsum))
